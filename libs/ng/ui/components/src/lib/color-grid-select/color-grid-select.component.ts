  ...
  private _onKeydown(event: KeyboardEvent) {
    switch (event.keyCode) {
      case UP_ARROW:{
        ...
        console.log(el?.id1.split('-'))
        const idsplit = el?.id1.split('-')
        var id =  'row-'+(Number(idsplit[1])-1) + '-item-' + idsplit[3]
        const elements = document.getElementsByTagName('brew-color-grid-item');
        var checked = true
        for (let i = 0; i < elements.length; i++) {
          ...
          if (element.id1 == id) {
            checked = false;
            element.focus()
            this.value = element.getAttribute('ng-reflect-value');
          }
        }
        if(checked){
          var el1:any = elements[elements.length -1]
             el1.id1.split('-')[1]
           let el:any = Array.from(elements).find((el:any)=>el.id1 == 'row-'+el1.id1.split('-')[1]+'-item-'+idsplit[3])
           if(el){
            el.focus();
            this.value = el.getAttribute('ng-reflect-value');
           }else{
      ...
      case DOWN_ARROW:
        {
          ...
          if(checked){
            let el:any = Array.from(elements).find((el:any)=>el.id1 == 'row-1-item-'+idsplit[3])
            if(el){
             el.focus();
             this.value = el.getAttribute('ng-reflect-value');
            }else{
